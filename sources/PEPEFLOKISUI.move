...
module PFS::PEPEFLOKISUI {
    ...
    /// The ticker of PFS regulated token
    struct PFS has drop {}

    /// A restricted transfer of PFS to another account.
    ...
    /// A PFSTreasuryCap for the balance::Supply.
    struct PFSTreasuryCap has key, store {
        id: UID,
        supply: Supply<PFS>
    }

    /// For when an attempting to interact with another account's RegulatedCoin<PFS>.
    const ENotOwner: u64 = 1;

    /// For when address has been banned and someone is trying to access the balance
    const EAddressBanned: u64 = 2;

    /// Create the PFS currency and send the PFSTreasuryCap to the creator
    /// as well as the first (and empty) balance of the RegulatedCoin<PFS>.
    ///
    /// Also creates a shared Registry which holds banned addresses.
    fun init(ctx: &mut TxContext) {
        let sender = tx_context::sender(ctx);
        let treasury_cap = PFSTreasuryCap {
            id: object::new(ctx),
            supply: balance::create_supply(PFS {})
        };
    ...
    /// Create an empty `RCoin<PFS>` instance for account `for`. PFSTreasuryCap is passed for
    /// authentication purposes - only admin can create new accounts.
    public entry fun create(_: &PFSTreasuryCap, for: address, ctx: &mut TxContext) {
        transfer::public_transfer(zero(for, ctx), for)
    }

    /// Mint more PFS. Requires PFSTreasuryCap for authorization, so can only be done by admins.
    public entry fun mint(treasury: &mut PFSTreasuryCap, owned: &mut RCoin<PFS>, value: u64) {
        balance::join(borrow_mut(owned), balance::increase_supply(&mut treasury.supply, value));
    }

    /// Burn `value` amount of `RCoin<PFS>`. Requires PFSTreasuryCap for authorization, so can only be done by admins.
    ///
    /// TODO: Make PFSTreasuryCap a part of Balance module instead of Coin.
    public entry fun burn(treasury: &mut PFSTreasuryCap, owned: &mut RCoin<PFS>, value: u64) {
    ...
    /// Only owner of the PFSTreasuryCap can perform this action.
    public entry fun ban(_cap: &PFSTreasuryCap, registry: &mut Registry, to_ban: address) {
    ...
    public entry fun transfer(r: &Registry, coin: &mut RCoin<PFS>, value: u64, to: address, ctx: &mut TxContext) {
        let sender = tx_context::sender(ctx);
    ...
    ///
    /// Fails if:
    /// 1. the `RegulatedCoin<PFS>.creator` does not match `Transfer.to`;
    /// 2. the address of the creator/recipient is banned;
    public entry fun accept_transfer(r: &Registry, coin: &mut RCoin<PFS>, transfer: Transfer) {
    ...
    ///
    /// Fails if:
    /// 1. `RegulatedCoin<PFS>.creator` was banned;
    /// 2. `RegulatedCoin<PFS>` is not owned by the tx sender;
    public entry fun take(r: &mut Registry, coin: &mut RCoin<PFS>, value: u64, ctx: &mut TxContext) {
        let sender = tx_context::sender(ctx);
    ...
    /// 1. `RegulatedCoin<PFS>.creator` was banned;
    /// 2. `RegulatedCoin<PFS>` is not owned by the tx sender;
    public entry fun put_back(r: &mut Registry, rc_coin: &mut RCoin<PFS>, coin: Coin<PFS>, ctx: &TxContext) {
    ...
    fun borrow(coin: &RCoin<PFS>): &Balance<PFS> { rcoin::borrow(PFS {}, coin) }
    fun borrow_mut(coin: &mut RCoin<PFS>): &mut Balance<PFS> { rcoin::borrow_mut(PFS {}, coin) }
    fun zero(creator: address, ctx: &mut TxContext): RCoin<PFS> { rcoin::zero(PFS {}, creator, ctx) }

    fun into_balance(coin: RCoin<PFS>): Balance<PFS> { rcoin::into_balance(PFS {}, coin) }
    fun from_balance(balance: Balance<PFS>, creator: address, ctx: &mut TxContext): RCoin<PFS> {
        rcoin::from_balance(PFS {}, balance, creator, ctx)
    }

    // === Testing utilities ===

    #[test_only] public fun init_for_testing(ctx: &mut TxContext) { init(ctx) }
    #[test_only] public fun borrow_for_testing(coin: &RCoin<PFS>): &Balance<PFS> { borrow(coin) }
    #[test_only] public fun borrow_mut_for_testing(coin: &mut RCoin<PFS>): &Balance<PFS> { borrow_mut(coin) }
}

#[test_only]
/// Tests for the PFS module. They are sequential and based on top of each other.
/// ```
/// * - test_minting
/// |   +-- test_creation
/// |       +-- test_transfer
...
/// ```
module PFS::tests {
    use PFS::PFS::{Self, PFS, PFSTreasuryCap, Registry};
    use rc::regulated_coin::{Self as rcoin, RegulatedCoin as RCoin};
    ...
    #[test]
    #[expected_failure(abort_code = PFS::PFS::EAddressBanned)]
    ...
    #[test]
    #[expected_failure(abort_code = PFS::PFS::ENotOwner)]
    ...
    // Admin creates a regulated coin PFS and mints 1,000,000 of it.
    fun test_minting_(test: &mut Scenario) {
        let (admin, _, _) = people();

        next_tx(test, admin);
        {
            PFS::init_for_testing(ctx(test))
        };

        next_tx(test, admin);
        {
            let cap = test_scenario::take_from_sender<PFSTreasuryCap>(test);
            let coin = test_scenario::take_from_sender<RCoin<PFS>>(test);

            PFS::mint(&mut cap, &mut coin, 1000000);
    ...
    fun test_transfer_(test: &mut Scenario) {
        ...
        next_tx(test, admin);
        {
            ...
            PFS::transfer(reg_ref, &mut coin, 500000, user1, ctx(test));
        ...
        {
            let coin = test_scenario::take_from_sender<RCoin<PFS>>(test);
            let transfer = test_scenario::take_from_sender<PFS::Transfer>(test);
            let reg = test_scenario::take_shared<Registry>(test);
            let reg_ref = &mut reg;

            PFS::accept_transfer(reg_ref, &mut coin, transfer);
    ...
    // Admin burns 100,000 of `RCoin<PFS>`
    fun test_burn_(test: &mut Scenario) {
        ...
        {
            let coin = test_scenario::take_from_sender<RCoin<PFS>>(test);
            let treasury_cap = test_scenario::take_from_sender<PFSTreasuryCap>(test);

            PFS::burn(&mut treasury_cap, &mut coin, 100000);
    ...
    // User1 sends Coin<PFS> it to `user2`.
    fun test_take_(test: &mut Scenario) {
        ...
        next_tx(test, user1);
        {
            ...
            PFS::take(reg_ref, &mut coin, 100000, ctx(test));

            assert!(PFS::swapped_amount(reg_ref) == 100000, 5);
        ...
        next_tx(test, user1);
        {
            let coin = test_scenario::take_from_sender<Coin<PFS>>(test);
            sui::transfer::public_transfer(coin, user2);
        };
    }

    // User2 sends his `Coin<PFS>` to `admin`.
    ...
    fun test_put_back_(test: &mut Scenario) {
        ...
        {
            let coin = test_scenario::take_from_sender<Coin<PFS>>(test);
            let reg_coin = test_scenario::take_from_sender<RCoin<PFS>>(test);
            let reg = test_scenario::take_shared<Registry>(test);
            let reg_ref = &mut reg;

            PFS::put_back(reg_ref, &mut reg_coin, coin, ctx(test));
    ...
    fun test_address_banned_fail_(test: &mut Scenario) {
        ...
        next_tx(test, user1);
        {
            ...
            PFS::transfer(reg_ref, &mut coin, 250000, user2, ctx(test));
    ...
    fun test_different_account_fail_(test: &mut Scenario) {
        ...
        next_tx(test, admin);
        {
            ...
            PFS::transfer(reg_ref, &mut coin, 250000, user1, ctx(test));
